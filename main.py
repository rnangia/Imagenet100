...
from JigPz import resnet as models
from JigPz.util import format_time
...
parser.add_argument('--loss-t', default=0.07, type=float,
                    help='softmax temperature (default: 0.07)')

# Jigsaw Puzzle 
parser.add_argument('--crop-size', default=112, type=int,
                    help='crop size for the patch')
parser.add_argument('--cross-ratio', default=0.3, type=float,
                    help='four patches crop cross ratio')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # create model
    if args.pretrained:
        progress.write("=> using pre-trained model '{}'".format(args.arch))
        model = models.__dict__[args.arch](pretrained=True)
    else:
        progress.write("=> creating model '{}'".format(args.arch))
        model = models.__dict__[args.arch]()
        progress.write(str(model))

    if not torch.cuda.is_available() and not torch.backends.mps.is_available():
        progress.write('using CPU, this will be slow')
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if args.gpu is not None:
                ...
                best_acc1 = torch.as_tensor(best_acc1, dtype=torch.float32, device=device)
            ...
            progress.write("=> no checkpoint found at '{}'".format(args.resume))


    # Data loading code
    if args.dummy:
        progress.write("=> Dummy data is used!")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best, filename='checkpoint_{:04d}.pth.tar'.format(epoch))
        info = 'Time: ' + format_time(time.time()-st_time) + '\n'\
               'Eta: ' + format_time((time.time()-st_time)/(epoch-args.start_epoch+1)*(args.epochs-epoch-1))
        progress.write(info)
